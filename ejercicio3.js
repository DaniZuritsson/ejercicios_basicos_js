// 3.1 Multiplica 10 por 5 y muestra el resultado mediante console.
let resultadoMultiplicacion = 10 * 5
console.log('Resultado de 10 * 5 es: ' + resultadoMultiplicacion)

// 3.2 Divide 10 por 2 y muestra el resultado en un console.
let resultadoDivision = 10 / 2
console.log('Resultado de 10 / 2 es: ' + resultadoDivision)

// 3.3 Muestra mediante un console el resto de dividir 15 por 9.
let restoDivision = 15 % 9
console.log('El resto de dividir 15 por 9 es: ' + restoDivision)

// 3.4 Usa el correcto operador de asignación que resultará en o = 15, teniendo dos variables p = 10 y j = 5.
let p = 10
let j = 5
let o = p + j // Suma para obtener o = 15
console.log('El valor de o es: ' + o)

// 3.5 Usa el correcto operador de asignación que resultará en i = 50, teniendo dos variables c = 10 y m = 5.
let c = 10
let m = 5
let i = c * m // Multiplicación para obtener i = 50
console.log('El valor de i es: ' + i)
